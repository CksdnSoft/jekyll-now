...
If the hacker found the actual plaintext the user uses as his password, then the hacker is just using classic brute force methods to figure out the password, which is not a concern in cryptography. There is no way to stop brute-force password searching or user. (It is rather a concern of security and user experience, where users must use a more secure password).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU4ODQ4NDU5MCwtMTcyODEwODE2Miw0OT
